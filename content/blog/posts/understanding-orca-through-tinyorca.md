+++
title = "Understanding Orca through tinyorca"
...
If you want more background on why batching makes each token cheaper to generate, I covered that in [Introduction to LLM Inference Part 1](./prefill-vs-decode.md).
...
$$
O_i = \mathrm{Attention}(Q_i, K_i^{(\mathrm{past+cur})}, V_i^{(\mathrm{past+cur})})
$$
